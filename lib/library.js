...
function openDB() {
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      // v3: `fit` used to default to an unused 'width'. The reader now honours it,
      // so normalise existing books to 'page' (fit-to-screen — today's behaviour).
      if (e.oldVersion < 3 && db.objectStoreNames.contains('comics')) {
        const store = req.transaction.objectStore('comics');
        store.openCursor().onsuccess = (ev) => {
          const cur = ev.target.result;
          if (!cur) return;
          const rec = cur.value;
          if (rec.fit !== 'page') { rec.fit = 'page'; cur.update(rec); }
          cur.continue();
        };
      }
    };
...
// Bulk-restore from an exported bundle. By default keeps anything already present
// (so re-importing won't clobber newer local reading positions). Returns the count
// of comics actually added.
export async function importLibraryData({ comics = [], files = {}, series = [] }) {
  const haveComic = new Set((await getAllComics()).map(c => c.id));
  const haveSeries = new Set((await getAllSeries()).map(s => s.key));
  let added = 0;
  await tx(['comics', 'files', 'series'], 'readwrite', ([cs, fs, ss]) => {
    for (const rec of comics) {
      if (!rec || !rec.id || haveComic.has(rec.id)) continue;
      cs.put(rec);
      if (files[rec.id]) fs.put({ id: rec.id, blob: files[rec.id] });
      added++;
    }
    for (const rec of series) {
      if (!rec || !rec.key || haveSeries.has(rec.key)) continue;
      ss.put(rec);
    }
  });
  return added;
