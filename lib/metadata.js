...
export function parseFilename(name) {
  ...
  for (const [re, k] of NUM_TOKENS) {
    ...
    if (mm) { number = String(parseInt(mm[1], 10)); kind = k; core = core.replace(re, ' '); matched = true; break; }
  }
  if (!matched) {
    const numM = core.match(/(?:^|\s)(\d{1,4})\s*$/); // trailing bare number
    if (numM) { number = String(parseInt(numM[1], 10)); core = core.slice(0, numM.index).trim(); }
  }
