...
//   { pageCount, format, comicInfo?, pdfInfo?, async getPageBlob(i), close() }
...
import { parseComicInfo } from './metadata.js';

const IMAGE_RE = /\.(jpe?g|png|gif|webp|avif|bmp)$/i;
const isComicInfo = p => /(^|\/)comicinfo\.xml$/i.test(p);
...
const EXT_BY_MIME = { 'image/jpeg': 'jpg', 'image/png': 'png', 'image/gif': 'gif', 'image/webp': 'webp', 'image/avif': 'avif', 'image/bmp': 'bmp' };
function extForBlob(b) {
  if (b.type && EXT_BY_MIME[b.type]) return EXT_BY_MIME[b.type];
  const m = (b.name || '').toLowerCase().match(/\.(jpe?g|png|gif|webp|avif|bmp)$/);
...
// Repackage an opened archive (CBR/CB7) into a CBZ blob so future reads use the
// fast, reliable zip path. Pages are STORED (level 0) — they're already
// compressed images, so this is fast and doesn't grow the file.
export async function archiveToCbz(backend) {
  const zip = window.zip;
  zip.configure({ useWebWorkers: false });
  const writer = new zip.ZipWriter(new zip.BlobWriter('application/zip'), { level: 0 });
  for (let i = 0; i < backend.pageCount; i++) {
    const blob = await backend.getPageBlob(i);
    await writer.add(`${String(i + 1).padStart(4, '0')}.${extForBlob(blob)}`, new zip.BlobReader(blob));
  }
  if (backend.comicInfoXml) { try { await writer.add('ComicInfo.xml', new zip.TextReader(backend.comicInfoXml)); } catch (_) {} }
  return writer.close();
}
...
async function openZip(blob) {
  ...
  let comicInfo = null;
  const ciEntry = all.find(e => !e.directory && isComicInfo(e.filename));
  if (ciEntry) { try { comicInfo = parseComicInfo(await ciEntry.getData(new zip.TextWriter())); } catch (_) {} }
  const entries = all
...
async function openArchive(blob) {
  ...
  const all = await archive.getFilesArray();
  let comicInfo = null, comicInfoXml = null;
  const ci = all.find(e => isComicInfo(e.path + e.file.name));
  if (ci) { try { comicInfoXml = await (await ci.file.extract()).text(); comicInfo = parseComicInfo(comicInfoXml); } catch (_) {} }
  const entries = all
...
async function openPdf(blob) {
  ...
  let pdfInfo = null;
  try { const m = await doc.getMetadata(); pdfInfo = m && m.info; } catch (_) {}
  return {
    format: 'pdf',
